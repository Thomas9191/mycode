...
int get_state_value(Board cur_board){
    ...
    for(int i=0;i < cur_board.SIZE;i++)for(int j=0;j<cur_board.SIZE;j++){
        ...
        if(cur_board.board[i][j] == player){
            ...
            if(corner.find(P(i,j)) != corner.end()) sum += 55;
            ...
            else if( ((i==0 || i==7) && ( j==2 || j==5)) || ((j==0 || j==7) && (i==2 || i==5))) sum += 14; 
            else if( (i==0 || i==7) && ( j==3 || j==4) || ((j==0 || j==7) && (i==3 || i==4))) sum += 10;
            else if( (i==2 || i==4) && ( j==2 || j==5)) sum += 11;
            else if( i==0 || i==7 || j==0 || j==7)sum += 5;
            else sum += 2;
        }

        else if(cur_board.board[i][j] == (3-player)){
            if(corner.find(P(i,j))!=corner.end())sum -= 70;
            else if(no_entry.find(P(i,j))!=no_entry.end())sum += 15;
            else if( ((i==0 || i==7) && ( j==2 || j==5)) || ((j==0 || j==7) && (i==2 || i==5))) sum -= 22; 
            else if(((i==0 || i==7) && ( j==3 || j==4)) || ((j==0 || j==7) && (i==3 || i==4))) sum -= 13;
            else if((i==2 || i==4) && ( j==2 || j==5)) sum -= 11;
            else if(i==0 || i==7 || j==0 || j==7)sum -= 7;
            else sum -= 3;
        }
    }
    if(cur_board.cur_player==player)return sum + (int)cur_board.next_valid_spots.size() * 2;
    else return sum - (int)cur_board.next_valid_spots.size() * 4;
}
int MinMax(Board cur , int depth , int alpha , int beta , int cur_player){
    if(depth==0) return get_state_value(cur);
